...
export default function AdminSidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-[900] flex h-screen w-64 flex-none flex-col border-r border-[#29352e] bg-[#111914] px-5 py-5 text-[#e8e2d5] shadow-[8px_0_30px_rgba(10,18,13,.08)] transition-[transform,width] duration-300 sm:sticky sm:top-0 sm:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"} ${collapsed ? "sm:w-20 sm:px-2" : "sm:w-60 sm:px-5"}`}
      >
        <button
          type="button"
          onClick={() => setMobileOpen(false)}
        ...
        <div className="mb-4 flex min-h-16 items-center justify-center">
          <Link
            href="/"
            className={`relative block h-16 w-44 -translate-x-2 transition-all sm:translate-x-0 ${collapsed ? "sm:h-14 sm:w-14" : "sm:h-16 sm:w-44"}`}
          >
            <Image
          ...
          {items.map(({ label, href, icon: Icon, dummy }) => {
            ...
            return (
              <Link
                ...
                className={`group relative flex items-center justify-start gap-3 rounded-xl px-3 py-3 text-sm font-medium transition ${collapsed ? "sm:justify-center" : ""} ${active ? "bg-[#c39a4b] text-[#111914] shadow-[0_8px_22px_rgba(195,154,75,.18)]" : dummy ? "cursor-default text-[#536158]" : "text-[#c6cec8] hover:bg-white/5 hover:text-[#e4bd70]"}`}
              >
                <Icon size={19} />
                <span className={collapsed ? "sm:hidden" : ""}>{label}</span>
                {dummy && (
                  <span
                    className={`ml-auto text-[9px] uppercase tracking-wide ${collapsed ? "sm:hidden" : ""}`}
                  >
                    Soon
                  </span>
                )}
        ...
        <Link
          ...
          className={`group relative mt-auto flex items-center justify-start gap-3 rounded-xl px-3 py-3 text-sm text-[#e4bd70] hover:bg-white/5 ${collapsed ? "sm:justify-center" : ""}`}
        >
          <Store size={19} />
          <span className={collapsed ? "sm:hidden" : ""}>View Storefront</span>
          {collapsed && (
            <span className="pointer-events-none absolute left-[calc(100%+12px)] top-1/2 z-50 hidden -translate-y-1/2 translate-x-1 whitespace-nowrap rounded-lg bg-forest-900 px-3 py-2 text-xs font-medium text-white opacity-0 shadow-lg transition-all duration-150 before:absolute before:right-full before:top-1/2 before:-translate-y-1/2 before:border-4 before:border-transparent before:border-r-forest-900 sm:block sm:group-hover:translate-x-0 sm:group-hover:opacity-100">
              View Storefront
            </span>
